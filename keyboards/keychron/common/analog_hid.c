...
case HID_CMD_SET_TRAVAL: {
    ...
    if (!entire) {
        uint8_t row = data[8];
        uint8_t col = data[9];
        row_mask[row] = (1UL << col);
    }

    bool success = profile_set_traval(
        profile,
        mode,
        act_pt,
        sens,
...
// set single key actuation point
// <row> <col> <act_pt 0–39, 0 = use global>
case HID_CMD_SET_KEY_ACT_PT: {
    if (length >= 4) {
        uint8_t row = data[1];
        uint8_t col = data[2];
        uint8_t act_pt = data[3];

        // Optional safety clamp
        if (act_pt > 39) act_pt = 39;

        analog_matrix_profile_t *cur_prof = profile_get_current();
        analog_key_config_t *p_key_cfg =
            &cur_prof->key_config[row][col];

        // 0 = inherit global actuation point
...
case HID_CMD_SET_KEY_RAPID: {
    if (length >= 5) {
        uint8_t row = data[1];
        uint8_t col = data[2];
        uint8_t press   = data[3];
        uint8_t release = data[4];

        // Optional safety clamps
        if (press > 39)   press = 39;
        if (release > 39) release = 39;

        analog_matrix_profile_t *cur_prof = profile_get_current();
        analog_key_config_t *p_key_cfg =
            &cur_prof->key_config[row][col];

        // Force rapid trigger mode
...
case HID_CMD_SET_OKMC: {
    ...
    uint8_t row        = data[1];
    uint8_t col        = data[2];
    uint8_t okmc_index = data[3];

    // 8-bit travel
    uint8_t shallow_act   = data[4];
    uint8_t shallow_deact = data[5];
    uint8_t deep_act      = data[6];
    uint8_t deep_deact    = data[7];

    // Keys (8-bit on wire)
    uint8_t key1 = data[8];
    uint8_t key2 = data[9];
    uint8_t key3 = data[10];
    uint8_t key4 = data[11];
    ...
    // Travel (8-bit → stock receiver)
    data_socd[5] = shallow_act;
    data_socd[6] = shallow_deact;
    data_socd[7] = deep_act;
    data_socd[8] = deep_deact;

    // Keys (16-bit LE)
    data_socd[9]  = key1; data_socd[10] = 0;
    data_socd[11] = key2; data_socd[12] = 0;
    data_socd[13] = key3; data_socd[14] = 0;
    data_socd[15] = key4; data_socd[16] = 0;

    // ACTION ENCODING
    data_socd[17] = ((a1 & 0x01) ? 0x02 : 0) | (((a1 & 0x02) ? 0x02 : 0) << 4);
    data_socd[18] = ((a1 & 0x04) ? 0x02 : 0) | (((a1 & 0x08) ? 0x02 : 0) << 4);

    data_socd[19] = ((a2 & 0x01) ? 0x02 : 0) | (((a2 & 0x02) ? 0x02 : 0) << 4);
    data_socd[20] = ((a2 & 0x04) ? 0x02 : 0) | (((a2 & 0x08) ? 0x02 : 0) << 4);

    data_socd[21] = ((a3 & 0x01) ? 0x02 : 0) | (((a3 & 0x02) ? 0x02 : 0) << 4);
    data_socd[22] = ((a3 & 0x04) ? 0x02 : 0) | (((a3 & 0x08) ? 0x02 : 0) << 4);

    data_socd[23] = ((a4 & 0x01) ? 0x02 : 0) | (((a4 & 0x02) ? 0x02 : 0) << 4);
    data_socd[24] = ((a4 & 0x04) ? 0x02 : 0) | (((a4 & 0x08) ? 0x02 : 0) << 4);
...
case HID_CMD_GET_CURVE: {
    bool success = game_controller_get_curve(data);

    if (success) {
        resp[0] = HID_CMD_GET_CURVE;
        resp[1] = 0x01;

        // 2 bytes per curve point (x, y)
        resp_len = CURVE_POINTS_COUNT * 2 + 2;

        memcpy(&resp[2], data, CURVE_POINTS_COUNT * 2);
...
case HID_CMD_SET_CURVE: {
    if (length >= 9) {  // 1 cmd + 4 points * 2 bytes
        point_t curve_points[4];

        // Directly assign 8-bit values
        for (int i = 0; i < 4; i++) {
            curve_points[i].x = data[1 + i*2];
            curve_points[i].y = data[1 + i*2 + 1];

            // Optional clamp (0–39)
            if (curve_points[i].x > 40) curve_points[i].x = 40;
            if (curve_points[i].y > 127) curve_points[i].y = 127;
        }

        // Set curve in firmware
        bool success = game_controller_set_curve(curve_points);

        resp[0] = HID_CMD_SET_CURVE;
        resp[1] = success ? 0x01 : 0x00;
    } else {
        resp[0] = HID_CMD_SET_CURVE;
        resp[1] = 0x00;
    }
