...
Usage examples (positional-only):
  ...
  lmk set-traval <profile 0-3> <mode0-4> <act_pt> <sens> <rls_sens> <entire 0/1> <row> <col> #act_pt/sens/rls_sens 0-40
  lmk set-act 0 1 20                # row/col/set actuation (0-40)
  lmk set-rapid 0 1 10 20            # row/col/press/release (0-40)
  ...
  lmk get-curve
  lmk set-curve x1 y1 x2 y2 x3 y3 x4 y4  (X 0-40 / Y 0-128)
  lmk calib-start
...
All numeric values that represent travel/X curve/rapid/actuation use 8-bit range 0..40.
"""
import sys
import time
import struct
...
def cmd_set_traval(dev, profile, mode, act_pt, sens, rls_sens, entire, row=None, col=None):
    # Clamp to valid 8-bit / QMK ranges
    act_pt   = max(0, min(39, act_pt))
    sens     = max(0, min(255, sens))
    rls_sens = max(0, min(255, rls_sens))

    payload = bytearray([
        24,        # HID_CMD_SET_TRAVAL
        0,         # reserved
        profile,
        mode,
        act_pt,    # 8-bit
    ...
    if not entire:
        if row is None or col is None:
            raise ValueError("When 'entire' is False, you must specify row and col.")
        payload.append(row)
        payload.append(col)
...
def cmd_set_key_act_pt(dev, row, col, act_pt):
    # Clamp to valid QMK range
    act_pt = max(0, min(39, act_pt))

    payload = bytearray([
        HID_CMD_SET_KEY_ACT_PT,
        row,
        col,
        act_pt  # 8-bit
    ])
...
def cmd_set_key_rapid(dev, row, col, press, release):
    # Clamp to valid rapid-trigger range
    press   = max(0, min(39, press))
    release = max(0, min(39, release))

    payload = bytearray([
        HID_CMD_SET_KEY_RAPID,
        row,
        col,
        press,
        release
...
def cmd_set_okmc(dev, row, col, idx,
    ...
    # Clamp travel values 0–39
    shallow_act   = max(0, min(39, shallow_act))
    shallow_deact = max(0, min(39, shallow_deact))
    deep_act      = max(0, min(39, deep_act))
    deep_deact    = max(0, min(39, deep_deact))

    # Payload: 8-bit sequential, no padding
    payload = bytearray([
        HID_CMD_SET_OKMC,
        row,
        col,
...
def cmd_set_curve(dev, pts):
    """
    pts: list of 8 integers [x1, y1, x2, y2, x3, y3, x4, y4]
    X = 0–40, Y = 0–128
    """
    clamped_pts = []
    for i, v in enumerate(pts):
        if i % 2 == 0:   # X
            clamped_pts.append(max(0, min(40, v)))
        else:            # Y
            clamped_pts.append(max(0, min(128, v)))

    payload = bytearray([HID_CMD_SET_CURVE])
    payload += bytearray(clamped_pts)
...
def pretty_get_curve(dev):
    ...
    r = cmd_get_curve(dev)
    if r and len(r) >= 2 and _ok_from_status_byte(r[1]):
        curve_data = r[2:]  # Remaining bytes are curve data
        print("Curve data received:")

        # 2 bytes per point: (x, y)
        num_points = len(curve_data) // 2
...
def main(argv):
    ...
    try:
        if cmd == "ping":
            ...
            mode = parse_int(argv[3], "mode", 0, 255)
            act_pt = parse_int(argv[4], "actuation point", 0, 39)
            sens = parse_int(argv[5], "sensitivity", 0, 39)
            rls_sens = parse_int(argv[6], "release sensitivity", 0, 39)
            ...
            val = parse_int(argv[4], "actuation", 0, 39)
            ...
            row = parse_int(argv[2], "row", 0, 255)
            col = parse_int(argv[3], "col", 0, 255)
            press = parse_int(argv[4], "press", 0, 39)
            release = parse_int(argv[5], "release", 0, 39)
            ...
            shallowA  = parse_int(argv[5],  "shallowA",   0, 40)
            shallowD  = parse_int(argv[6],  "shallowD",   0, 40)
            deepA     = parse_int(argv[7],  "deepA",      0, 40)
            deepD     = parse_int(argv[8],  "deepD",      0, 40)
            ...
            if len(argv) < 10:
                raise SystemExit("usage: lmk set-curve x1 y1 x2 y2 x3 y3 x4 y4")
            pts = []
            for i, x in enumerate(argv[2:10]):
                if i % 2 == 0:  # X values (travel)
                    pts.append(parse_int(x, None, 0, 40))
                else:           # Y values (curve factor)
                     pts.append(parse_int(x, None, 0, 128))
